...
また、指定した時間に、強制的に EPG 取得を行う機能も盛り込んだ。 (が、少し想定とは違う動作をしている。正確には、起動した直後に自動的に取得が行われるため、指定時刻より前に取得が開始される。目標は達成できているのでよしとしておく。)
...
設定項目:
    ...
    http://localhost:20772      # Chinachu WUI のアドレス (最後に "/" はつけないこと。wuiOpenPort の方じゃないとダメかも。)
